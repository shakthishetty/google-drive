...
const Search = () => {
  ...
  const handleClickItem = (file:Models.Document)=>{
     setOpen(false)
     setResult([])
     router.push(`/${file.type === 'video' || file.type === 'audio' ? 'media' : file.type + 's'}?query}`);
         ...
         <ul className="search-result">
          ...
          result.map((file) => 
            ...
            >
               ...
               <FromattedDateTime date={file.$createdAt} className="caption line-clamp-1 text-light-200"/>
           </li>)
          ):(
